    ...
    if (sCheckNdefStatus == NFA_STATUS_FAILED)
    {
        ...
        if (sCheckNdefCapable)
        {
            ...
            if (sCurrentConnectedTargetProtocol == NFA_PROTOCOL_MIFARE)
            {
                ...
                if (status != NFA_STATUS_OK)
                {
                    LOG(ERROR) << StringPrintf("%s: can't format mifare classic tag", __func__);
...
static int reSelect (tNFA_INTF_TYPE rfInterface, bool fSwitchIfNeeded)
{
    DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf("%s: enter; rf intf = %d, current intf = %d", __func__, rfInterface, sCurrentRfInterface);
    ...
    do
    {
...

        {
            ...
            DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf("%s: select interface %u", __func__, rfInterface);
        ...
        DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf("%s: select completed; sConnectOk=%d", __func__, sConnectOk);
    ...
    sRfInterfaceMutex.unlock ();
    DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf("%s: exit; status=%d", __func__, rVal);
    return rVal;
}
...
static bool switchRfInterface (tNFA_INTF_TYPE rfInterface)
{
    ...
    if (sCurrentConnectedTargetProtocol != NFC_PROTOCOL_ISO_DEP)
    {
        DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf("%s: protocol: %d not ISO_DEP, do nothing", __func__, natTag.mTechLibNfcTypes[0]);
        return true;
    }
